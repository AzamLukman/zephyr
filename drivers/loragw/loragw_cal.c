...
int sx1302_cal_start(const struct device *dev, uint8_t version, struct lgw_conf_rxrf_s * rf_chain_cfg, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    sx1302_agc_mailbox_write(dev, 3, 0xFF);

    /* Wait for AGC to acknoledge */
    sx1302_agc_wait_status(dev, 0x00);
    ...
    lgw_reg_w(dev, SX1302_REG_RADIO_FE_IQ_COMP_AMP_COEFF_RADIO_A_AMP_COEFF, (int32_t)rf_rx_image_amp[0]);
    lgw_reg_w(dev, SX1302_REG_RADIO_FE_IQ_COMP_PHI_COEFF_RADIO_A_PHI_COEFF, (int32_t)rf_rx_image_phi[0]);
    lgw_reg_w(dev, SX1302_REG_RADIO_FE_IQ_COMP_AMP_COEFF_RADIO_B_AMP_COEFF, (int32_t)rf_rx_image_amp[1]);
    lgw_reg_w(dev, SX1302_REG_RADIO_FE_IQ_COMP_PHI_COEFF_RADIO_B_PHI_COEFF, (int32_t)rf_rx_image_phi[1]);
...
// int sx125x_cal_tx_dc_offset(const struct device *dev, uint8_t rf_chain, uint32_t freq_hz, uint8_t dac_gain, uint8_t mix_gain, uint8_t radio_type, struct lgw_sx125x_cal_tx_result_s * res) {
...
//             return LGW_HAL_ERROR;
//     }
//     sx125x_reg_w(dev, SX125x_REG_FRF_RX_MSB, 0xFF & rx_freq_int, rf_chain);
...
//     sx1302_agc_mailbox_write(dev, 3, 0x0c + 20); /* sync */
//     /* -----------------------------------------------*/
...
//     sx1302_agc_wait_status(dev, (rf_chain == 0) ? 0x33 : 0x44);

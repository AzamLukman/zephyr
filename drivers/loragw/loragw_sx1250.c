...
int sx1250_reg_w(const struct device *dev, sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain) {
    ...
    com_stat = sx1250_com_w(dev, ((rf_chain == 0) ? LGW_SPI_MUX_TARGET_RADIOA : LGW_SPI_MUX_TARGET_RADIOB), op_code, data, size);
...
int sx1250_reg_r(const struct device *dev, sx1250_op_code_t op_code, uint8_t *data, uint16_t size, uint8_t rf_chain) {
    ...
    com_stat = sx1250_com_r(dev, ((rf_chain == 0) ? LGW_SPI_MUX_TARGET_RADIOA : LGW_SPI_MUX_TARGET_RADIOB), op_code, data, size);
...
int sx1250_calibrate(const struct device *dev, uint8_t rf_chain, uint32_t freq_hz) {
    ...
    buff[0] = 0x00;
    err |= sx1250_reg_r(dev, GET_SX1250_STATUS, buff, 1, rf_chain);
    ...
    err |= sx1250_reg_w(dev, CALIBRATE_IMAGE, buff, 2, rf_chain);
    ...
    buff[1] = 0x00;
    buff[2] = 0x00;
    err |= sx1250_reg_r(dev, GET_DEVICE_ERRORS, buff, 3, rf_chain);
...
int sx1250_setup(const struct device *dev, uint8_t rf_chain, uint32_t freq_hz, bool single_input_mode) {
    ...
    err |= sx1250_reg_w(dev, SET_STANDBY, buff, 1, rf_chain);
    wait_ms(10);
    ...
    buff[0] = 0x7F;
    err |= sx1250_reg_w(dev, CALIBRATE, buff, 1, rf_chain);
    wait_ms(10);
    ...
    buff[1] = 0xA1;
    buff[2] = 0x01;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain);
    buff[0] = 0x06;
    buff[1] = 0xA2;
    buff[2] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain);
    ...
    buff[1] = 0x82;
    buff[2] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain); /* Drive strength to min */
    buff[0] = 0x05;
    buff[1] = 0x83;
    buff[2] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain); /* Input enable, all disabled */
    buff[0] = 0x05;
    buff[1] = 0x84;
    buff[2] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain); /* No pull up */
    buff[0] = 0x05;
    buff[1] = 0x85;
    buff[2] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain); /* No pull down */
    buff[0] = 0x05;
    buff[1] = 0x80;
    buff[2] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain); /* Output enable, all enabled */
    ...
    err |= sx1250_reg_w(dev, SET_RF_FREQUENCY, buff, 4, rf_chain);
    ...
    buff[3] = 0x00;
    buff[4] = 0x00;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 5, rf_chain);

    /* Set Radio in Rx mode, necessary to give a clock to SX1302 */
    buff[0] = 0xFF;
    buff[1] = 0xFF;
    buff[2] = 0xFF;
    err |= sx1250_reg_w(dev, SET_RX, buff, 3, rf_chain); /* Rx Continuous */
    ...
    buff[1] = 0x87;
    buff[2] = 0x0B;
    err |= sx1250_reg_w(dev, WRITE_REGISTER, buff, 3, rf_chain); /* FPGA_MODE_RX */

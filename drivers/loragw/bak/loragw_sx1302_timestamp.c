...
int timestamp_counter_get(const struct device *dev, timestamp_counter_t * self, uint32_t * inst, uint32_t * pps) {
    int x;
    uint8_t buff[8];
    uint8_t buff_wa[8];
    ...
    */
    x = lgw_reg_rb(dev, SX1302_REG_TIMESTAMP_TIMESTAMP_PPS_MSB2_TIMESTAMP_PPS, &buff[0], 8);
     ...
     */
    x = lgw_reg_rb(dev, SX1302_REG_TIMESTAMP_TIMESTAMP_PPS_MSB2_TIMESTAMP_PPS, &buff_wa[0], 8);
    if (x != LGW_REG_SUCCESS) {
        return -1;
    }
...
int timestamp_counter_mode(const struct device *dev, bool ftime_enable) {
    ...
    if (ftime_enable == false) {
        ...
        x |= lgw_reg_w(dev, SX1302_REG_RX_TOP_RX_BUFFER_LEGACY_TIMESTAMP, 0x01);
    } else {
        ...
        x |= lgw_reg_w(dev, SX1302_REG_RX_TOP_RX_BUFFER_LEGACY_TIMESTAMP, 0x00);
        x |= lgw_reg_w(dev, SX1302_REG_RX_TOP_RX_BUFFER_TIMESTAMP_CFG_MAX_TS_METRICS, (int32_t)PRECISION_TIMESTAMP_TS_METRICS_MAX);

        /* LoRa multi-SF modems */
        x |= lgw_reg_w(dev, SX1302_REG_RX_TOP_TIMESTAMP_ENABLE, 0x01);
        x |= lgw_reg_w(dev, SX1302_REG_RX_TOP_TIMESTAMP_NB_SYMB, (int32_t)PRECISION_TIMESTAMP_NB_SYMBOLS);
    }
...
int precise_timestamp_calculate(const struct device *dev, uint8_t ts_metrics_nb, const int8_t * ts_metrics, uint32_t timestamp_cnt, uint8_t sf, int32_t if_freq_hz, double pkt_freq_error, uint32_t * result_ftime) {
    ...
    x = lgw_reg_rb(dev, SX1302_REG_TIMESTAMP_TIMESTAMP_PPS_MSB2_TIMESTAMP_PPS , &buff[0], 4);
    if (x != LGW_REG_SUCCESS) {

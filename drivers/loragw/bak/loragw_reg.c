...
static int lgw_check_ver(const struct device *dev)
{
	...
	com_stat = lgw_com_r(dev, LGW_SPI_MUX_TARGET_SX1302, loregs[SX1302_REG_COMMON_VERSION_VERSION].addr, &u);
...
int lgw_reg_wb(const struct device *dev, uint16_t register_id, uint8_t *data, uint16_t size) {
    int com_stat = LGW_COM_SUCCESS;
    struct lgw_reg_s r;
    ...
    /* do the burst write */
    com_stat = lgw_com_wb(dev, LGW_SPI_MUX_TARGET_SX1302, r.addr, data, size);
...
int lgw_reg_rb(const struct device *dev, uint16_t register_id, uint8_t *data, uint16_t size) {
    int com_stat = LGW_COM_SUCCESS;
    struct lgw_reg_s r;
    ...
    /* do the burst read */
    com_stat = lgw_com_rb(dev, LGW_SPI_MUX_TARGET_SX1302, r.addr, data, size);
...
int lgw_mem_wb(const struct device *dev, uint16_t mem_addr, const uint8_t *data, uint16_t size) {
    int com_stat = LGW_COM_SUCCESS;
    int chunk_cnt = 0;
    ...
    while (sz_todo > 0) {
        ...
        /* do the burst write */
        com_stat = lgw_com_wb(dev, LGW_SPI_MUX_TARGET_SX1302, addr, &data[chunk_cnt * CHUNK_SIZE_MAX], chunk_size);
...
int lgw_mem_rb(const struct device *dev, uint16_t mem_addr, uint8_t *data, uint16_t size, bool fifo_mode) {
    int com_stat = LGW_COM_SUCCESS;
    int chunk_cnt = 0;
    uint16_t addr = mem_addr;
    uint16_t sz_todo = size;
    ...
    while (sz_todo > 0) {
        ...
        /* do the burst read */
        com_stat = lgw_com_rb(dev, LGW_SPI_MUX_TARGET_SX1302, addr, &data[chunk_cnt * CHUNK_SIZE_MAX], chunk_size);

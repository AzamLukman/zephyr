...
const struct lgw_reg_s loregs[1044+1] = {
...
int sx1302_get_eui(const struct device *dev, uint64_t * eui) {
    ...
    *eui = 0;
    for (i = 0; i < 8; i++) {
        err = lgw_reg_w(dev, 826, i);
        if (err != LGW_COM_SUCCESS) {
            return LGW_SPI_ERROR;
        }
        err = lgw_reg_r(dev, 827, &val);
        if (err != LGW_COM_SUCCESS) {
            return LGW_SPI_ERROR;
        }
...
static int loragw_init(const struct device *dev)
{
	...
	// 	  if (x != LGW_COM_SUCCESS) {
        // printk("ERROR: failed to get model ID\n");
    	// }
	// printk("MODEL ID: %X\n", (sx1302_model_id_t)model_id);
	...
	x = lgw_com_w(dev, LGW_SPI_MUX_TARGET_SX1302, SX1302_AGC_MCU_MEM, test_buff);
	printk("\ntest_buff = %X\n", test_buff);
	if (x != 0){
	...
	x = lgw_com_r(dev, LGW_SPI_MUX_TARGET_SX1302, SX1302_AGC_MCU_MEM, &read_buff);
	printk("read_buff = %X\n", read_buff);
	if (x != 0){

...
int lgw_start(const struct device *dev) {
    int i, err;
...
//     }
    ...
    /* Calibrate radios */
    err = sx1302_radio_calibrate(dev, &CONTEXT_RF_CHAIN[0], CONTEXT_BOARD.clksrc, &CONTEXT_TX_GAIN_LUT[0]);
    if (err != LGW_REG_SUCCESS) {
        ...
        if (CONTEXT_RF_CHAIN[i].enable == true) {
            ...
            switch (CONTEXT_RF_CHAIN[i].type) {
                case LGW_RADIO_TYPE_SX1250:
                    err = sx1250_setup(dev, i, CONTEXT_RF_CHAIN[i].freq_hz, CONTEXT_RF_CHAIN[i].single_input_mode);
                    break;
    ...
    err = sx1302_radio_clock_select(dev, CONTEXT_BOARD.clksrc);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_radio_host_ctrl(dev, false);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_init(dev, &CONTEXT_FINE_TIMESTAMP);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_pa_lna_lut_configure(dev, &CONTEXT_BOARD);
    if (err != LGW_REG_SUCCESS) {
    ...
    /* Configure Radio FE */
    err = sx1302_radio_fe_configure(dev);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_channelizer_configure(dev, CONTEXT_IF_CHAIN, false);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_lora_correlator_configure(dev, CONTEXT_IF_CHAIN, &(CONTEXT_DEMOD));
    if (err != LGW_REG_SUCCESS) {
        printf("ERROR: failed to configure SX1302 LoRa modem correlators\n");
        return LGW_HAL_ERROR;
    }
    err = sx1302_lora_modem_configure(dev, CONTEXT_RF_CHAIN[0].freq_hz);
    if (err != LGW_REG_SUCCESS) {
    ...
    if (CONTEXT_IF_CHAIN[8].enable == true) {
        err = sx1302_lora_service_correlator_configure(dev, &(CONTEXT_LORA_SERVICE));
        if (err != LGW_REG_SUCCESS) {
            printf("ERROR: failed to configure SX1302 LoRa Service modem correlators\n");
            return LGW_HAL_ERROR;
        }
        err = sx1302_lora_service_modem_configure(dev, &(CONTEXT_LORA_SERVICE), CONTEXT_RF_CHAIN[0].freq_hz);
        if (err != LGW_REG_SUCCESS) {
    ...
    if (CONTEXT_IF_CHAIN[9].enable == true) {
        err = sx1302_fsk_configure(dev, &(CONTEXT_FSK));
        if (err != LGW_REG_SUCCESS) {
    ...
    /* configure syncword */
    err = sx1302_lora_syncword(dev, CONTEXT_LWAN_PUBLIC, CONTEXT_LORA_SERVICE.datarate);
    if (err != LGW_REG_SUCCESS) {
    ...
    switch (CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type) {
        case LGW_RADIO_TYPE_SX1250:
            ...
            err = sx1302_agc_load_firmware(dev, agc_firmware_sx1250);
            if (err != LGW_REG_SUCCESS) {
        ...
        //     err = sx1302_agc_load_firmware(dev, agc_firmware_sx125x);
        //     if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_agc_start(dev, FW_VERSION_AGC, CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type, SX1302_AGC_RADIO_GAIN_AUTO, SX1302_AGC_RADIO_GAIN_AUTO, CONTEXT_BOARD.full_duplex, CONTEXT_SX1261.lbt_conf.enable);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_arb_load_firmware(dev, arb_firmware);
    if (err != LGW_REG_SUCCESS) {
        printf("ERROR: failed to load ARB firmware\n");
        return LGW_HAL_ERROR;
    }
    err = sx1302_arb_start(dev, FW_VERSION_ARB, &CONTEXT_FINE_TIMESTAMP);
    if (err != LGW_REG_SUCCESS) {
    ...
    err = sx1302_tx_configure(dev, CONTEXT_RF_CHAIN[CONTEXT_BOARD.clksrc].type);
    if (err != LGW_REG_SUCCESS) {
    ...
    /* enable GPS */
    err = sx1302_gps_enable(dev, true);
    if (err != LGW_REG_SUCCESS) {
...
int lgw_stop(const struct device *dev) {
...
int lgw_receive(const struct device *dev, uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    int res;
...
//     }
    ...
    for (nb_pkt_found = 0; nb_pkt_found < ((nb_pkt_fetched <= max_pkt) ? nb_pkt_fetched : max_pkt); nb_pkt_found++) {
        ...
        res = sx1302_parse(dev, &lgw_context, &pkt_data[nb_pkt_found]);
...
int lgw_send(const struct device *dev, struct lgw_pkt_tx_s * pkt_data) {
    int err;
    bool lbt_tx_allowed;
    /* performances variables */
    struct timeval tm;
...
//     }
    ...
    err = sx1302_send(dev, CONTEXT_RF_CHAIN[pkt_data->rf_chain].type, &CONTEXT_TX_GAIN_LUT[pkt_data->rf_chain], CONTEXT_LWAN_PUBLIC, &CONTEXT_FSK, pkt_data);
    if (err != LGW_REG_SUCCESS) {
...
//             err = sx1302_tx_abort(dev, pkt_data->rf_chain);
//             if (err != 0) {
...
int lgw_status(const struct device *dev, uint8_t rf_chain, uint8_t select, uint8_t *code) {
    DEBUG_PRINTF(" --- %s\n", "IN");
    ...
    if (select == TX_STATUS) {
        if (CONTEXT_STARTED == false) {
            ...
            *code = sx1302_tx_status(dev, rf_chain);
        }
    } else if (select == RX_STATUS) {
        if (CONTEXT_STARTED == false) {
            *code = RX_OFF;
        } else {
            *code = sx1302_rx_status(dev, rf_chain);
        }
    } else {
...
int lgw_abort_tx(const struct device *dev, uint8_t rf_chain) {
    int err;
    ...
    /* Abort current TX */
    err = sx1302_tx_abort(dev, rf_chain);
...
int lgw_get_trigcnt(const struct device *dev, uint32_t* trig_cnt_us) {
    DEBUG_PRINTF(" --- %s\n", "IN");

    CHECK_NULL(trig_cnt_us);

    *trig_cnt_us = sx1302_timestamp_counter(dev, true);
...
int lgw_get_instcnt(const struct device *dev, uint32_t* inst_cnt_us) {
    DEBUG_PRINTF(" --- %s\n", "IN");

    CHECK_NULL(inst_cnt_us);

    *inst_cnt_us = sx1302_timestamp_counter(dev, false);
...
int lgw_get_eui(const struct device *dev, uint64_t* eui) {
    DEBUG_PRINTF(" --- %s\n", "IN");

    CHECK_NULL(eui);

    if (sx1302_get_eui(dev, eui) != LGW_REG_SUCCESS) {
        return LGW_HAL_ERROR;
    }

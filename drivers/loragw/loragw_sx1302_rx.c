...
int rx_buffer_fetch(const struct device *dev, rx_buffer_t * self) {
    ...
    lgw_reg_rb(dev, SX1302_REG_RX_TOP_RX_BUFFER_NB_BYTES_MSB_RX_BUFFER_NB_BYTES, buff, sizeof buff);
    ...
    if (self->buffer_size > 0) {
        memset(self->buffer, 0, sizeof self->buffer);
        res = lgw_mem_rb(dev, 0x4000, self->buffer, self->buffer_size, true);
...
uint16_t rx_buffer_read_ptr_addr(const struct device *dev) {
    ...
    lgw_reg_r(dev, SX1302_REG_RX_TOP_RX_BUFFER_LAST_ADDR_READ_MSB_LAST_ADDR_READ, &val); /* mandatory to read MSB first */
    addr  = (uint16_t)(val << 8);
    lgw_reg_r(dev, SX1302_REG_RX_TOP_RX_BUFFER_LAST_ADDR_READ_LSB_LAST_ADDR_READ, &val);
    addr |= (uint16_t)val;
...
uint16_t rx_buffer_write_ptr_addr(const struct device *dev) {
    ...
    lgw_reg_r(dev, SX1302_REG_RX_TOP_RX_BUFFER_LAST_ADDR_WRITE_MSB_LAST_ADDR_WRITE, &val);  /* mandatory to read MSB first */
    addr  = (uint16_t)(val << 8);
    lgw_reg_r(dev, SX1302_REG_RX_TOP_RX_BUFFER_LAST_ADDR_WRITE_LSB_LAST_ADDR_WRITE, &val);
    addr |= (uint16_t)val;

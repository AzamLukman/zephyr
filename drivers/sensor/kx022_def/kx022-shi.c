...
static int kx022_init(const struct device *dev)
{
	...
	val = (KX022_DEFAULT_RES << KX022_CNTL1_RES_SHIFT) |
	      (KX022_DEFAULT_FS << KX022_CNTL1_GSEL_SHIFT);
...
#define KX022_DEVICE_INIT(inst)                                                                    \
	DEVICE_DT_INST_DEFINE(inst, kx022_init, NULL, &kx022_data_##inst, &kx022_config_##inst,    \
...
#define KX022_CONFIG_I2C(inst)                                                                     \
	{                                                                                          \
		.comm_master_dev_name = DT_INST_BUS_LABEL(inst), .bus_init = kx022_i2c_init,       \
		KX022_CFG_IRQ(inst)                                                                \
	}
#define KX022_DEFINE_I2C(inst)                                                                     \
	static struct kx022_data kx022_data_##inst;                                                \
	static const struct kx022_config kx022_config_##inst = KX022_CONFIG_I2C(inst);             \
	KX022_DEVICE_INIT(inst)

#define KX022_DEFINE(inst)                                                                         \
	COND_CODE_1(DT_INST_ON_BUS(inst,spi),			\
		    NULL,					\
		    (KX022_DEFINE_I2C(inst))

DT_INST_FOREACH_STATUS_OKAY(KX022_DEFINE_I2C)

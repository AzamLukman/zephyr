...
static int kx022_sample_fetch_accel_xyz(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(data, KX022_REG_XHP_L, buf, sizeof(buf)) < 0) {
...
static int kx022_init(const struct device *dev)
{
	...
	// #if DT_NODE_HAS_STATUS(DT_NODELABEL(accel_ps), okay)

	// 	TURN_DO_(accel_ps, _OFF);
	// 	LOG_WRN("acc POWER OFF");
	// 	k_msleep(100);
	// 	TURN_DO_(accel_ps, _ON);
...
#define KX022_CONFIG_I2C(inst)					\
	{                                                       \
		.comm_master_dev_name = DT_INST_BUS_LABEL(inst),\
		.bus_init = kx022_i2c_init,       		\
		KX022_CFG_IRQ(inst)				\
	}
